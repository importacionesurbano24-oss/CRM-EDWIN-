...
export default async function ClientesPage({
  ...
  const clientesPorEtapa = todosLosClientes.filter(
    (c) => filtroEtapas.length === 0 || (c.etapa && filtroEtapas.includes(c.etapa))
  );
  const clientes = clientesPorEtapa.filter((c) =>
  ...
  return (
    <ClientesView
      ...
      clientes={clientes}
      clientesParaConteoFecha={clientesPorEtapa}
